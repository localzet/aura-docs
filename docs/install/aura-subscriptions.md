...
# Aura Subscription Page

Aura Subscription Page is lightweight and secure way to hide your Aura Panel domain from end users. You can use it as a simple and beautiful subscription page for your users.

![Page screenshot](/install/subscriptions.webp)

## Step 1 - Prepare .env file

Edit the `/opt/aura/.env` file and change `SUB_PUBLIC_DOMAIN` to your subscription page domain name.
...
mkdir -p /opt/aura/subscription && cd /opt/aura/subscription && nano docker-compose.yml
```
...
Please replace `panel.com` with the URL at which the **Aura Dashboard** is available.

`REMNAWAVE_PANEL_URL` can be http (bridged Aura, example: `http://aura-backend:3000`) or https (example: `https://panel.com`).
...
Aura and its components does not support being server on a sub-path. (e.g. `location /subscription {`)
...
```bash
cd /opt/aura/subscription && nano docker-compose.yml
```

:::warning

Please use the docker container name and port (`http://aura-backend:3000`) instead of the panel URL (`https://panel.com`).
...
```bash
acme.sh --issue --standalone -d 'SUBSCRIPTION_PAGE_DOMAIN' --key-file /opt/aura/nginx/subdomain_privkey.key --fullchain-file /opt/aura/nginx/subdomain_fullchain.pem --alpn --tlsport 8443
```

Open Nginx configuration file:

```bash
cd /opt/aura/nginx && nano nginx.conf
```
...
```bash
cd /opt/aura/nginx && nano docker-compose.yml
```
...
If you have already configured Traefik, you need to create a new dynamic configuration file called `aura-subscriptions.yml` in the `/opt/aura/traefik/config` directory.

```bash
cd /opt/aura/traefik/config && nano aura-subscriptions.yml
```
...
```yaml title="aura-subscriptions.yml"
http:
  routers:
    aura-subscriptions:
      // highlight-next-line-yellow
...
This guide explains how to add client applications for different platforms to the Aura subscription page.
...
- **The `index.html` file and all files in the `assets` directory must be mounted into the container at the following paths:**
    ...
    [subscriptions/frontend/index.html](https://github.com/localzet/aura-subscriptions/blob/main/frontend/index.html)

    The `assets` directory is available here:
    [subscriptions/frontend/public/assets](https://github.com/localzet/aura-subscriptions/tree/main/frontend/public/assets)
    :::
...
Restart the subscriptions container to apply the changes.
...
Modify your docker-compose.yml file to mount the app-config.json file to the subscriptions container:
...
See the [complete example](https://raw.githubusercontent.com/localzet/aura-subscriptions/refs/heads/main/frontend/public/assets/app-config.json) to understand how to configure multiple applications across different platforms.
