...
In this guide we will be using Nginx as a reverse proxy to access the Aura panel.
...
```bash
mkdir -p /opt/aura/nginx && cd /opt/aura/nginx
```
...
```bash
acme.sh --issue --standalone -d 'DOMAIN' --key-file /opt/aura/nginx/privkey.key --fullchain-file /opt/aura/nginx/fullchain.pem --alpn --tlsport 8443
```
...
Create a file called `nginx.conf` in the `/opt/aura/nginx` directory.
...
Create a `docker-compose.yml` file in the `/opt/aura/nginx` directory.
