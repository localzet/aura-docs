...
Aura will strictly enforce the limit on the number of devices that can add the subscription.
...
Aura will return a `404` error if no HWID header is sent.
:::
...
Aura is using these headers to identify the HWID and the device.
...
If your application has the ability to enable additional features based on where the subscription is coming from, Aura can send a provider id in the response headers, which you can use to figure out where the subscription is coming from.
